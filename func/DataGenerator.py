...
from func.img_process import _img_augmentation, _convert_to_np_array, _img_mixup

class DataGenerator_mixup(keras.utils.Sequence):
...
#         print(int(np.floor((self.df.shape[0]) / self.batch_size)))
    ...
    def __getitem__(self, index):
        ...
        sample_df = self.df.sample(n=self.batch_size)
        sample_df["img"] = [np.array(load_img("../data/train/{}.png".format(idx), grayscale=True)) / 255 for idx in sample_df.id]
        sample_df["img_mask"] = [np.array(load_img("../data/masks/{}.png".format(idx), grayscale=True)) / 255 for idx in sample_df.id]
              
        mixup_df = _img_mixup(batch_df, sample_df, if_augment=self.augment)
        X_np, y_np = _convert_to_np_array(mixup_df)
